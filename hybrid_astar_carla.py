...
def hybrid_astar_planning(car, sx, sy, syaw, gx, gy, gyaw, ox, oy, xyreso, yawreso):
    ...
    while True:
        ...
        **Extracted from the original paper**
        "For computational rea- sons, it is not desirable to apply the Reed-Shepp
        expansion to every node (especially far from the goal, where most such
        paths are likely to go through obstacles). In our implementa- tion, we
        used a simple selection rule, where the Reed-Shepp expansion is applied to 
        one of every N nodes, where N de- creases as a function of the cost-to-goal
        heuristic (leading to more frequent analytic expansions as we get closer to the goal)."
...
# Respect the holonomic structure but ignores obstacles
